...
import { ContainerDiv, FormDiv, HeaderDiv } from '../components/styles/FromStyle'
...
const BoardDetail = () => {
  ...
  ])//하나는 파일명 - bs_file, 하나는 파일크기 - bs_size 를 배열로 관리
  ...
  useEffect(()=>{
    const boardDetail = async() => {
      ...
      setBoard({BM_NO:jsonDoc[0].BM_NO,BM_TITLE:jsonDoc[0].BM_TITLE,BM_WRITER:jsonDoc[0].BM_WRITER
              ...
              ,BM_HIT:jsonDoc[0].BM_HIT,/* BM_FILE:jsonDoc[0].BM_FILE,BM_SIZE:jsonDoc[0].BM_SIZE */
      })
      //첨부파일,quill editor 이미지 파일
      setFiles([jsonDoc[0].BM_FILE,jsonDoc[0].BM_SIZE])
    }
    boardDetail()
  },[pboard])//의존성 배열에는 제목에 해당되는 bm_no가 변경될 때마다 새로 실행됨
  //의존성배열을 적지 않으면 변경될 때마다 새로 읽는다 - 초기화 된다
  //빈배열이면 최초 App(BoardDetail)이 렌더링될 때 딱 한번만 실행됨
  //주의사항 : boards와 같은 n개 로우를 갖는 변수명을 사용하지 말것 - 무한루프(리랜더링) - setBoards
  //재렌더링 - state, props, 부모가 바뀔 때
