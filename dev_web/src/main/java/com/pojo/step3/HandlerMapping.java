...
public class HandlerMapping {
	 ...
	 * @param request                                            -1-1, 1-2와는 다르게
	 *                                                           인터페이스를 implements
	 *                                                           하지 않는다
	 * @param response                                           질문? 어디서 받아오지? 서블릿에서
	 *                                                           받아와야지.. 근데 이건 넘
	 *                                                           추상적..
	 * @return Object
	 * 
	...
	public static Object getController(String[] upmu, HttpServletRequest req, HttpServletResponse res)
		...
		if ("board3".equals(upmu[0])) {
			controller = new Board3Controller();// 인스턴스화
			// 게시글 전체 목록
			if ("boardList".equals(upmu[1])) { // html 화면 출력 나감 - text/html
				obj = controller.boardList(req, res);
				// 리턴타입이 ModelAndWView
				if (obj instanceof ModelAndView) { // 타입체크
					return (ModelAndView) obj;
					// 리턴타입이 String
				} else if (obj instanceof String) { // 타입체크
					return (String) obj;
				}
			} else if ("jsonBoardList".equals(upmu[1])) { // json포맷으로 나감 - application/jon
				// 리턴타입이 String
			...
			} else if ("boardDetail".equals(upmu[1])) { // 글입력 - 새글쓰기와 댓글쓰기
				obj = controller.boardDetail(req, res);
				logger.info("boardDetail 호출 ==> boolean " + obj instanceof String);
				if (obj instanceof String) {
					return (String) obj;
				}
